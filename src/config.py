...
SYSTEM_PROMPT = (
    ...
    # ── location 규칙 ──────────────────────────────────────────────────
    "실제 행사·교육·대회·오디션이 열리는 특정 장소(건물명, 강의실, 기관명, 주소)나 단일 지역명이 명확히 제시된 경우에만 location에 넣으세요. "
    "'부천시 일원', '파주시 주요 공연장 및 공공장소', '전국 각지', '여러 장소', '추후 공지', '개별 안내'처럼 "
    "특정 장소가 확정되지 않은 표현은 location에 넣지 말고 빈 문자열 \"\"로 쓰세요. "
    "온라인 접수, 이메일 제출, 구글폼 신청, 홈페이지 신청, SNS 업로드 자체는 location이 아닙니다. "
    "행사 자체가 온라인으로 진행되는 경우에만 location을 \"온라인\"으로 쓰세요. "
    "장소 정보가 전혀 없거나 불특정 장소이면 location은 빈 문자열 \"\"로 쓰세요. "
    # ── detail 규칙 ─────────────────────────────────────────────────────
